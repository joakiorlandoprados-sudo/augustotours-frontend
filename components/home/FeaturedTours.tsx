...
function FeaturedTourCard({ tour, index }: { tour: Tour; index: number }) {
  const cardRef = useRef<HTMLElement>(null);
  const [isActive, setIsActive] = useState(false);

  // Si el usuario toca fuera de esta card, desactivar su carrusel.
  useEffect(() => {
    if (!isActive) return;
  ...
  return (
    <section id="featured" className="container-page py-20">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {featured.map((tour, i) => (
          <FeaturedTourCard key={tour.id} tour={tour} index={i} />
        ))}
      </div>
    </section>
  );
